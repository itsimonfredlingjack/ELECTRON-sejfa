...
export function MissionHeader({ objectiveText, connected }: MissionHeaderProps) {
    ...
    return (
        <div className="flex items-center gap-4 px-6 py-3">
            ...
            {/* Ticket badge */}
            <span className={`shrink-0 font-mono text-sm font-bold tracking-tight px-2.5 py-0.5 rounded border transition-all duration-500 ${connected
                    ? 'text-cyan-400 bg-cyan-400/10 border-cyan-400/20'
                    : 'text-red-400 bg-red-400/10 border-red-400/20 text-glitch'
                }`} data-text={ticketId}>
                {ticketId}
            </span>
